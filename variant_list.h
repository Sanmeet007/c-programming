#ifndef VLIST_HEADER_FILE
#define VLIST_HEADER_FILE
...
using VariantListType = std::variant<int, double, float, long long, long, char, string>;
typedef VariantListType vlt;
...
public:
    ...
    void each(const std::function<void(size_t, T)> &cb)
    {
        size_t i = 0;
        Node<T> *ptr = start;
        while (ptr != NULL)
    ...
    VariantList *slice(size_t start, size_t stop, size_t step = 1)
    {
        size_t i = 0, next_index = start;
        VariantList<T> *li = new VariantList<T>();
        if (this->start != NULL)
        {
    ...
    VariantList *map(const std::function<T(size_t, T)> &cb)
    {
        size_t i = 0;
        VariantList<T> *li = new VariantList<T>();
        Node<T> *ptr = start;
    ...
    VariantList *filter(const std::function<bool(size_t, T)> &cb)
    {
        size_t i = 0;
        VariantList<T> *li = new VariantList<T>();
        Node<T> *ptr = start;
